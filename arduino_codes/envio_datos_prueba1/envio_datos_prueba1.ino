...
// =========================
// RESPIRA+ Fase 5
// AP + HTTP diagnostico + WebSocket + VL53L0X real
// =========================
...
const unsigned long SENSOR_READ_INTERVAL_MS = 20;  // 50 lecturas/s
const unsigned long WS_SEND_INTERVAL_MS = 50;      // 20 mensajes/s
const unsigned long STATUS_INTERVAL_MS = 5000;     // log cada 5 s
...
<body>
  <div class="container">
    ...
    <p class="subtitle">Página local servida desde el ESP32. Esta fase valida datos reales del VL53L0X.</p>
    ...
    <div class="grid">
      ...
      <div class="card">
        <div class="label">distanceMm</div>
...
void sendRawSensorJson() {
  ...
  snprintf(
    jsonBuffer,
    sizeof(jsonBuffer),
    "{\"source\":\"%s\",\"volumeMl\":%d,\"sustainedTimeMs\":%d,\"validRepetitions\":%d,\"distanceMm\":%d,\"flowState\":\"%s\",\"isValidAttempt\":%s,\"timestamp\":%lu}",
    source,
    volumeMl,
    sustainedTimeMs,
    validRepetitions,
...
void setup() {
  ...
  Serial.println();
  Serial.println("================================");
  Serial.println("RESPIRA+ Fase 5");
  Serial.println("AP + HTTP diagnostico + WebSocket + VL53L0X real");
  ...
  if (!lox.begin()) {
    ...
    Serial.println("No se iniciara WiFi hasta corregir el sensor.");
